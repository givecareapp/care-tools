...
# Code Review: care-tools

## Project Context

This is a **documentation-only** repository — no runtime code exists yet. It contains the GC-SDOH-30 assessment framework (markdown specs), an adaptive assessment pattern document, and scaffolding for agent-driven development (Ralph, Specify). The review therefore focuses on framework correctness, documentation quality, and readiness for implementation.

---

## Code Quality (Framework & Content Bugs)

### 1. Zone ordering is inconsistent in `GC-SDOH.md`
...
### 2. Scoring contradiction for reverse-scored items
...
### 3. Scoring scale mismatch between documents
...
### 4. Question 29 scoring direction is ambiguous
...
### 5. Question 30 uses a different scale type
...
### 6. Constitution is an unfilled template
...
### 7. `CONTRIBUTING.md` referenced but missing
...
### 9. `GC-SDOH.md:321` has placeholder contact info

The contact section reads `[Contact information]` — a visible placeholder in what's otherwise a publication-ready document.

### 10. Example spec still present
...
### 11. No scoring validation exists
...
### 12. Adaptive assessment lacks concrete question mapping
...
### 1. Fix Q29 reverse-scoring omission
...
### 2. Add a scoring reference table with worked examples
...
### 3. Reconcile risk threshold scales between documents
**Impact: High | Effort: Low**
...
### 4. Fill in the constitution and fix placeholder URLs
...
### 5. Reorder questions by zone in `GC-SDOH.md`
**Impact: Medium | Effort: Low**
